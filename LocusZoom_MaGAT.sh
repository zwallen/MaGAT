...
# (https://my.locuszoom.org/) from MaGAT or Meta_MaGAT results.      #
# Note: For Meta_MaGAT results, only will work if SNP IDs are in the #
# format CHR:POS:REF:ALT                                             #
# Last updated: 12 May 2021                                          #
#                                                                    #
# Usage ./Format_LocusZoom.sh -i result_file \                       #
#                             -t type_of_results \                   #
...
#     -i    (Required) Result file from running MaGAT or Meta_MaGAT  #
...
#           Only valid when -t is Meta_MaGAT and will be ignored     #
#           otherwise.                                               #
...
while getopts ":hi:t:v:m:o:" opt; do
  case $opt in
    h)
    echo " "
    echo " Usage ./Format_LocusZoom.sh -i result_file \                       "
    ...
    echo "     -i    (Required) Result file from running MaGAT or Meta_MaGAT  "
    ...
    echo "           Only valid when -t is Meta_MaGAT and will be ignored     "
    echo "           otherwise.                                               "
...
# -i
if [[ -z "$IN_FILE" ]]; then
  echo "Argument -i is required, please supply a result file from running MaGAT or Meta_MaGAT."
  exit 1
fi
